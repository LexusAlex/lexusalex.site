...
В итоге получаем полный адаптив.
...
Во фремворке bootstrap 5 используется 12 колоночная сетка с 6 контрольными точками.

Класс `row` является флекс контейнером элементы которого можно переносить на следующую строку, а входящие в него элементы - флекс элементами

````html
<div class="row"></div>
````

Если внутрь положить элементы, то элементы будут занимать ширину в `100%`. Это обычное поведение.

Чтобы управлять колонкой нужно добавить класс `col` у которого добавлено свойство `flex: 1 0 0%;`

Что значит:

- `1` - колонка растет при наличии свободного места
- `0` - сжатие колонки
- `0%` - базовый размер колонки

Таким образом мы сделаем колонки одинаковой ширины
