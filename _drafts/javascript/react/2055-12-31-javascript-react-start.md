...
Состояние в `React` (`state`) - это данные которые меняются и влияют на отображение компонента или комопонентов.

Состояние может быть локальным и и глобальным
...
Что можно вынести к данному моменту:

- Определяем начальное состояние `state`.
- Меняем `state` когда это нужно методом `setState`.
- Перерисовываем компонент.

Перерисовку комопонента, например мы можем производить с помощью `setInterval` вызывая функцию `changeTime` каждую секунду.
...
> Данные должны быть доступны всему приложению, тогда подойдет использование глобального сотояния.
{: .prompt-info }
