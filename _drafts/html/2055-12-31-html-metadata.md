...
Элемент `<base>` определяет базовый `url` относительно которого будут работать ссылки, скрипты, изображения.
И указывает на целевое окно или фрейм где будут открыты ссылки.
...
- Если `<base>` все же используется нудно расположить его как можно выше к в теге `<head>`.
